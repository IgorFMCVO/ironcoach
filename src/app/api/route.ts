// ============================================================================
// API: ENVIAR PARA PERSONAL E REATIVAR
// Adicione estas rotas no seu arquivo de API ou crie um novo
// Caminho sugerido: src/app/api/personal/route.ts
// ============================================================================
...
// ============================================================================
// POST - Enviar para Personal ou Reativar
// ============================================================================
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'sendToPersonal') {
      ...
      if (error) {
        console.error('Erro ao enviar para personal:', error);
    ...
    console.error('Erro na API de personal:', error);
    return NextResponse.json(
...
/*
// Enviar para Personal (quando notifica como "com personal")
const sendToPersonal = async (queueId: string) => {
  const response = await fetch('/api/personal', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      action: 'sendToPersonal',
      queueId,
  ...
  if (data.success) {
    toast.success('Aluno enviado para Personal/Autônomo');
    refreshQueue();
  }
};

// Reativar (botão REATIVAR no card PURPLE)
const reactivateCard = async (queueId: string) => {
  const response = await fetch('/api/personal', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  const data = await response.json();
  if (data.success) {
    toast.success(`Aluno reativado como ${data.new_priority}`);
    refreshQueue();
  }
};
*/
